...
# train function for the GreenSwarm model
def train_model_gs(env, BATCH_SIZE):
...
# evaluate function for the GreenSwarm model  
def evaluate_model_gs(env, x_test, y_test, BATCH_SIZE):
...
def train_network_gs(env, dataset_name, x_test, y_test, BATCH_SIZE, EPOCHS, input_size, output_size, optimizer, loss):
    ...
    print()
    print("GreenSwarm Model Training... ")
    print()
    
    for node in env.nodes:
        node.define_model(dataset_name, input_size, output_size, optimizer, loss) # initialize the model at each node
        
    # loop over the number of epochs   
    for epoch_num in range(EPOCHS):
    
        avg_train_loss, avg_train_accuracy = train_model_gs(env, BATCH_SIZE) # perform one round of training

        avg_test_loss, avg_test_accuracy = evaluate_model_gs(env, x_test, y_test, BATCH_SIZE) # perform one round of evaluation
